...
public class ManualBackup {
    ...
    //backup each file seperate each time it is backed up.
    Calendar now = Calendar.getInstance();
    String cname = System.getProperty("user.home");

    public void Backup(){
        try {
            String folder = "backup"+now.get(Calendar.YEAR)+now.get(Calendar.MONTH);
            new File(folder).mkdir();
            //String path = cname+"\\HS Results\\backup"+now.get(Calendar.YEAR)+now.get(Calendar.MONTH)+".sql";
            //String path = "backup\\backup"+now.get(Calendar.YEAR)+now.get(Calendar.MONTH)+".sql";
            String path = "backup"+now.get(Calendar.YEAR)+now.get(Calendar.MONTH)+"\\backup"+now.get(Calendar.YEAR)+now.get(Calendar.MONTH)+".sql";
